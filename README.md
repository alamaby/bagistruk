...
- **Per-bill currency** — Bills snapshot their ISO currency at save time, so changing the default currency later does not re-label older bills. Plus users can override currency per bill during review, with support for ASEAN currencies plus INR, JPY, CNY, USD, EUR, GBP, RUB, BRL, MXN, AUD, and SAR.
